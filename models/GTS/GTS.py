...
import torch
import numpy as np
from configs.GTS_configs import GTSConfig
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config: GTSConfig):
        ...
        self.seq_len = config.input_len  # for the encoder
...
class GTS(AbstractSTEncoder, Seq2SeqAttrs):
    def __init__(self, config: GTSConfig, gp_supports, scaler):
        """
        构造模型
        ...
        super(GTS, self).__init__(config, gp_supports, scaler)
        self.config = config
        self.adj_mx = gp_supports[0]
        Seq2SeqAttrs.__init__(self, self.config)

        self.seq_len = config.input_len  # for the encoder
        self.horizon = config.output_len  # for the decoder

        self.encoder_model = EncoderModel(self.config)
        self.decoder_model = DecoderModel(self.config)
        self._logger = getLogger()

        # 此处 adj_mx 作用是在训练自动图结构推断时起到参考作用
        self.adj_mx = torch.Tensor(gp_supports[0]).to(self.device)
        # print(f"ADJMX={self.adj_mx}")
        self.cl_decay_steps = config.cl_decay_steps
        self.use_curriculum_learning = False
        self.temperature = config.temperature
        self.epoch_use_regularization = config.epoch_use_regularization

        self.num_batches = config.num_batches
        self.feature_dim = self.input_dim
        self.output_dim = config.output_len
        self.ext_dim = 0
        train_feas = self.data_feature.get('train_data')  # (num_samples, num_nodes)  # TODO

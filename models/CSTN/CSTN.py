...
class TEC(nn.Module):
    def __init__(self, c_in, c_lt):
        super(TEC, self).__init__()
        self.ConvLSTM = ConvLSTM(
            input_dim=c_in, hidden_dim=32, kernel_size=(3, 3), num_layers=1, batch_first=True)
...
class CSTN(AbstractSTEncoder):
    def __init__(self, config: CSTNConfig, gp_supports, scaler):
        super(CSTN, self).__init__(config, gp_supports, scaler)
    ...
    def forward(self, x, gp_supports, trues):
        # x: N V V L -> N L HW HW
        x = x.permute(0, 3, 1, 2)
        x = x.view(self.batch_size, self.input_window, self.height, self.width, self.height, self.width)
        ...
        x = self.OUT(x)
        # x : (B, H * W, H, W)
        x = x.reshape((self.batch_size, self.height * self.width, self.height * self.width))
        # x : N V V

...
class STDecomposition(Module):
    def __init__(self, config: TrainingArguments, supports):
        ...
        for i in range(self.tk):
            for j in range(self.sk):
                self.mds.append(MDBlock(copy.deepcopy(config), supports, i, j))
                self.node_emb.append(nn.Parameter(torch.randn(self.num_nodes, self.num_nodes), requires_grad=True))
    ...
    def forward(self, x):
        ...
        for i in range(self.tk):
            # N C V V l
            for j in range(self.sk):
                pred.append(self.mds[i * self.sk + j](x[i][j], self.node_emb[i * self.sk + j]))
    ...
    def decomposition(self, x):
        ...
        x = ptwt.wavedec(x, wavelet, mode='zero', level=self.tk)
